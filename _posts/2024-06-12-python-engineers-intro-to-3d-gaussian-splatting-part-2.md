...
- **Gaussian parameterization.** Each splat is defined by 11 parameters: 3D coordinates, a quaternion (4 values) that becomes a rotation matrix, 3 scale values, and an opacity float. The covariance matrix is decomposed as RSS'R' which guarantees it's always positive semi-definite, avoiding the optimization headaches of trying to learn a raw covariance matrix directly
- **Projecting covariance to 2D.** We know how to project points to 2D from Part 1, but we also need to project the 3D covariance matrix. This uses the EWA Splatting formulation with a Jacobian affine transformation. The z-axis information gets discarded after projection since we only care about the 2D image plane
- **Switching to OpenGL projection.** Part 2 swaps the intrinsic matrix from Part 1 for an OpenGL perspective projection matrix. The two are equivalent for point projection, but the OpenGL version matches the original author's CUDA code more closely
